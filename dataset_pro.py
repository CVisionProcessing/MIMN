...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        _class = self._class[self.fwflow_list[item].split('/')[-2]]
...
class TestDataset(Dataset):
    def __init__(self, path, size):
        ...
        if self.path.split('/')[-1]=='DAVIS':
            ...
            for i in seqs:
                ...
                self.fwflow_list+=sorted(glob.glob(os.path.join(path, "raft_flow", i, "*.png")))
        # fbms
        elif self.path.split('/')[-1]=='FBMS':
            with open('dataset/FBMS/val_seqs.txt') as f:
                seqs = f.readlines()
                seqs = [seq.strip() for seq in seqs]
            print(seqs)
            cnt=0
            for i in seqs:
                if i not in self._class:
                    self._class[i]=cnt
                    cnt+=1
                self.img_list+=sorted(glob.glob(os.path.join(path, "Testset", i, "*.jpg")))[:-1]
                self.fwflow_list+=sorted(glob.glob(os.path.join(path, "raft_flow", i, "*.png")))
        # segtrackv2
        elif self.path.split('/')[-1]=='SegTrackv2':
            with open('dataset/SegTrackv2/ImageSets/all.txt') as f:
                seqs = f.readlines()
                seqs = [seq.strip() for seq in seqs]
            print(seqs)
            cnt=0
            for i in seqs:
                if i not in self._class:
                    self._class[i]=cnt
                    cnt+=1
                self.img_list+=sorted(glob.glob(os.path.join(path, "JPEGImages", i, "*.jpg")))[:-1]
                self.fwflow_list+=sorted(glob.glob(os.path.join(path, "raft_flow", i, "*.png")))
    ...
    def __getitem__(self, item):
        ...
        _class = self._class[self.fwflow_list[item].split('/')[-2]]
        seq = self.fwflow_list[item].split('/')[-2]
        name = self.fwflow_list[item].split('/')[-1][:-4]

        inputSize = (video.shape[1], video.shape[0])
